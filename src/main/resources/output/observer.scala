...
class Observer(val DEFAULT_BOARD_X: Int, val DEFAULT_BOARD_Y: Int){
	...
	def main():Unit = {
		...
		while(true){
			...
			set_go_m(tmp_9)
			val tmp_10 = DEFAULT_observer.get_turtles().toList.map(s => asyncMessage(() => s.lambda_0(this)))
			while(!tmp_10.forall(_.isCompleted)){
				waitAndReply(1)
			}
			println("middle of tick")
			val tmp_11 = DEFAULT_observer.get_turtles().toList.map(s => asyncMessage(() => s.lambda_2(this)))
